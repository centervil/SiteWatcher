...
1. **最初のステップとして**、今日の日付（YYYY-MM-DD形式）を取得し、開発日記ファイル「Documents/ProjectLogs/YYYY-MM-DD-[テーマ].md」を作成してください。
   - テーマは私が最初に伝えます
   - ファイル名は必ず英語で作成してください（例：2024-03-27-zenn-article-format-fix.md）
...
## 前日までの活動履歴
Documents/ProjectLogsディレクトリ内の日付を確認し、関連する最近の活動を箇条書きで記載してください。
例：
```
前日までの活動履歴は以下の通りです：
* 2024-03-26: タイムスタンプファイルの扱いについて改善
* 2024-03-25: Frontend Dockerfile Volumes 設定とホットリロード
* 2024-03-24: Frontend タイムスタンプ表示位置修正、JST対応
```
...
5. 会話ログの記録については、詳細なフォーマットと記録ルールを「Documents/conversation_log_format.md」に定義しています。このファイルを参照してください。
